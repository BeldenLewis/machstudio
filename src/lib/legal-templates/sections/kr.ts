...
const PRIVACY_SECTIONS: Section[] = [
  ...
  },
  {
    ...
    render: () =>
      `## 6. 개인정보의 국외 이전\n\n` +
      `${orgLineKo()}가 운영하는 등록 시스템의 특성상 이용자의 개인정보가 ${ORG_TOKEN.hostingRegion}의 ` +
      `서버에 저장·처리될 수 있습니다. 회사는 국외 이전이 발생하는 경우 관계 법령이 정한 절차와 ` +
      `보호조치를 따릅니다.`,
  },
  {
  ...
  },
  {
    ...
    render: (ctx) => {
      const childData =
        `### 8-1. 만 14세 미만 아동의 개인정보\n\n` +
        `회사는 만 14세 미만 아동의 개인정보를 수집하는 경우 「개인정보 보호법」 제22조의2에 따라 ` +
        `법정대리인의 동의를 받으며, 이때는 동의 확인에 필요한 최소한의 정보(법정대리인의 성명, ` +
        `연락처 등)만 수집합니다. 법정대리인은 그 아동의 개인정보에 대해 열람, 오류 정정, 삭제, ` +
        `처리정지를 요구할 권리를 아동 본인을 대신하여 행사할 수 있습니다. 만 14세 미만 아동이 ` +
        `법정대리인의 동의 없이 개인정보를 제공한 사실을 알게 되신 경우 "문의처"로 연락해 주시면 ` +
        `지체 없이 확인 후 삭제 조치하겠습니다.`;

      if (ctx.event.adultsOnly) {
        return (
          `## 8. 아동·미성년자 개인정보 보호\n\n${childData}\n\n` +
          `### 8-2. 성인 전용 안내\n\n` +
          `${eventName(ctx)}은(는) 만 19세 이상 성인만 참가할 수 있는 대회입니다. 만 19세 미만인 ` +
          `분은 참가 신청을 할 수 없으며, 신청 과정에서 만 19세 미만임이 확인되는 경우 신청이 ` +
          `제한되거나 취소될 수 있습니다.`
        );
      }
      ...
      return (
        `## 8. 아동·미성년자 개인정보 보호\n\n${childData}\n\n` +
        `### 8-2. 만 19세 미만 미성년자의 참가\n\n` +
        `「민법」상 성년은 만 19세이며, 만 19세 미만 미성년자가 하는 법률행위(참가 신청, ` +
        `이용약관·개인정보 처리 동의 등 포함)는 원칙적으로 법정대리인의 동의를 받아야 유효하게 ` +
        `성립합니다. 법정대리인의 동의 없이 이루어진 신청은 미성년자 본인 또는 법정대리인이 ` +
        `취소할 수 있습니다. 이에 따라 회사는 만 19세 미만 참가자에게 법정대리인 동의서(또는 ` +
        `이에 준하는 확인 자료) 제출을 요청할 수 있으며, 제출하지 않을 경우 참가 신청이 제한될 ` +
        `수 있습니다. 상금·부상의 지급, 제출 자료 및 현장 촬영 영상의 이용(위 "제출 자료·현장 ` +
        `촬영 영상의 이용" 참고)과 같이 참가에 수반되는 절차에도 법정대리인의 동의가 함께 필요할 ` +
        `수 있습니다.`
      );
    },
  },
