...
describe("로케일 — 저장은 맵, 페이로드는 문자열", () => {
  it("요청 로케일의 문자열만 나간다", () => {
    ...
    const ko = buildExpoPayload(sections, ctx([], "ko"));
    ...
    const en = buildExpoPayload(sections, ctx([], "en"));
  ...
  it("줄바꿈은 그대로 나간다", () => {
    ...
    const out = buildExpoPayload(sections, ctx([]));
...
describe("내부 링크 — 같은 사이트의 살아 있는 페이지만", () => {
  ...
  it("아임웹 주소가 있으면 그 주소로 푼다", () => {
    const out = buildExpoPayload(withLink("page:p1"), ctx([{ id: "p1", imwebUrl: "https://expo.test/directions" }]));
  ...
  it("아임웹 주소가 없으면 비우고 알린다", () => {
    const out = buildExpoPayload(withLink("page:p1"), ctx([{ id: "p1", imwebUrl: null }]));
  ...
  it("다른 사이트의 페이지 id 는 풀리지 않는다", () => {
    const out = buildExpoPayload(withLink("page:다른사이트페이지"), ctx([{ id: "p1", imwebUrl: "https://expo.test/x" }]));
    expect(linkOf(out.sections[0], "cta").href).toBe("");
    expect(JSON.stringify(out)).not.toContain("expo.test");
  });

  it("외부 https 링크는 그대로 나간다", () => {
    const out = buildExpoPayload(withLink("https://외부.test/a"), ctx([]));
