...
import { ArrowLeft, BarChart3, ChevronRight, ImageIcon, Layers3, Loader2, Megaphone, PlayCircle, PlugZap, RefreshCw, Search, X } from "lucide-react";
...
type Folder = { id: string; name: string; description: string | null; reportStart: string; reportEnd: string; currency: string; resultMetric: string; detailColumns: unknown; mediaAccounts: Array<{ platform: string; accountId: string; accountName: string }> };
type DataRow = { id: string; sourceType: string; name: string; campaignId: string | null; campaignName: string; adGroupId: string | null; adGroupName: string | null; adId: string | null; creativeId: string | null; creativeName: string | null; thumbnailUrl: string | null; creativeType: string | null; status: string | null; cost: number; impressions: number; reach: number; clicks: number; conversions: number; ctr: number; cpc: number; costPerConversion: number };
...
export default function FolderDetailClient({ folderId }: { folderId: string }) {
  const [folder, setFolder] = useState<Folder>({ id: folderId, name: "", description: null, reportStart: "", reportEnd: "", currency: "KRW", resultMetric: "lead", detailColumns: [], mediaAccounts: [] });
  ...
  const [preview, setPreview] = useState<{ row: DataRow; html: string | null; loading: boolean } | null>(null);
  ...
  async function openPreview(row: DataRow) {
    ...
    setPreview({ row, html: null, loading: row.sourceType === "META" && Boolean(row.adId) });
    if (row.sourceType !== "META" || !row.adId) return;
    try {
      const response = await fetch(`/api/ad-performance/folders/${folderId}/ad-preview?adId=${encodeURIComponent(row.adId)}`);
      const data = await response.json().catch(() => null);
      if (!response.ok) throw new Error(data?.error || "미리보기를 가져오지 못했습니다.");
      setPreview({ row, html: data.html, loading: false });
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "미리보기를 가져오지 못했습니다.");
  ...
  return <div>
    ...
    {tab === "overview" ? <AdPerformanceDashboard key={`${folder.reportStart}:${folder.reportEnd}:${folder.resultMetric}:${JSON.stringify(folder.detailColumns)}`} folderId={folderId} folderProject={project} reportStart={folder.reportStart} reportEnd={folder.reportEnd} resultMetric={normalizeMetaResultMetric(folder.resultMetric)} detailColumns={normalizeAdDetailColumns(folder.detailColumns)} /> : tab === "connections" ? <FolderConnectionsPanel folderId={folderId} folderName={folder.name} folderDescription={folder.description} reportStart={folder.reportStart} reportEnd={folder.reportEnd} resultMetric={normalizeMetaResultMetric(folder.resultMetric)} detailColumns={normalizeAdDetailColumns(folder.detailColumns)} accounts={folder.mediaAccounts ?? []} onFolderNameChange={name => setFolder(current => ({ ...current, name }))} onFolderDescriptionChange={description => setFolder(current => ({ ...current, description }))} onDateRangeChange={range => setFolder(current => ({ ...current, ...range }))} onDashboardSettingsChange={settings => setFolder(current => ({ ...current, ...settings }))} onChange={mediaAccounts => setFolder(current => ({ ...current, mediaAccounts }))} /> : false ? (
