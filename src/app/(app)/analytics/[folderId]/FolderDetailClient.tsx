...
export default function FolderDetailClient({ folderId }: { folderId: string }) {
  ...
  return <div>
    ...
    {tab === "overview" ? <AdPerformanceDashboard folderId={folderId} folderProject={project} /> : tab === "connections" ? <FolderConnectionsPanel folderId={folderId} folderName={folder.name} reportStart={folder.reportStart} reportEnd={folder.reportEnd} accounts={folder.mediaAccounts ?? []} onFolderNameChange={name => setFolder(current => ({ ...current, name }))} onDateRangeChange={range => setFolder(current => ({ ...current, ...range }))} onChange={mediaAccounts => setFolder(current => ({ ...current, mediaAccounts }))} /> : false ? (
