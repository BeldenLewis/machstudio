...
type Folder = { id: string; name: string; description: string | null; reportStart: string; reportEnd: string; currency: string; detailColumns: unknown; mediaAccounts: Array<{ platform: string; accountId: string; accountName: string }> };
type DataRow = { id: string; sourceType: string; name: string; campaignId: string | null; campaignName: string; adGroupId: string | null; adGroupName: string | null; adId: string | null; creativeId: string | null; creativeName: string | null; thumbnailUrl: string | null; creativeType: string | null; status: string | null; cost: number; impressions: number; reach: number; clicks: number; conversions: number; ctr: number; cpc: number; cpm: number; costPerConversion: number };
...
export default function FolderDetailClient({ folderId }: { folderId: string }) {
  const [folder, setFolder] = useState<Folder>({ id: folderId, name: "", description: null, reportStart: "", reportEnd: "", currency: "KRW", detailColumns: [], mediaAccounts: [] });
  ...
  return <div>
    ...
    {tab === "overview" ? <AdPerformanceDashboard key={`${folder.reportStart}:${folder.reportEnd}:${JSON.stringify(folder.detailColumns)}`} folderId={folderId} folderProject={project} reportStart={folder.reportStart} reportEnd={folder.reportEnd} detailColumns={normalizeAdDetailColumns(folder.detailColumns)} /> : tab === "connections" ? <FolderConnectionsPanel folderId={folderId} folderName={folder.name} folderDescription={folder.description} reportStart={folder.reportStart} reportEnd={folder.reportEnd} detailColumns={normalizeAdDetailColumns(folder.detailColumns)} accounts={folder.mediaAccounts ?? []} onFolderNameChange={name => setFolder(current => ({ ...current, name }))} onFolderDescriptionChange={description => setFolder(current => ({ ...current, description }))} onDateRangeChange={range => setFolder(current => ({ ...current, ...range }))} onDashboardSettingsChange={detailColumns => setFolder(current => ({ ...current, detailColumns }))} onChange={mediaAccounts => setFolder(current => ({ ...current, mediaAccounts }))} /> : false ? (
