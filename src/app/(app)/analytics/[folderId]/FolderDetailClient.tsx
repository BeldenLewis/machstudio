...
export default function FolderDetailClient({ folderId }: { folderId: string }) {
  ...
  return <div>
    ...
    {tab === "overview" ? <AdPerformanceDashboard folderId={folderId} folderProject={project} /> : tab === "connections" ? <FolderConnectionsPanel folderId={folderId} folderName={folder.name} accounts={folder.mediaAccounts ?? []} onFolderNameChange={name => setFolder(current => ({ ...current, name }))} onChange={mediaAccounts => setFolder(current => ({ ...current, mediaAccounts }))} /> : false ? (
