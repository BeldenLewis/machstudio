...
export default function FolderDetailClient({ folderId }: { folderId: string }) {
  ...
  return <div>
    ...
    {tab === "overview" ? <AdPerformanceDashboard key={`${folder.reportStart}:${folder.reportEnd}`} folderId={folderId} folderProject={project} reportStart={folder.reportStart} reportEnd={folder.reportEnd} /> : tab === "connections" ? <FolderConnectionsPanel folderId={folderId} folderName={folder.name} folderDescription={folder.description} reportStart={folder.reportStart} reportEnd={folder.reportEnd} accounts={folder.mediaAccounts ?? []} onFolderNameChange={name => setFolder(current => ({ ...current, name }))} onFolderDescriptionChange={description => setFolder(current => ({ ...current, description }))} onDateRangeChange={range => setFolder(current => ({ ...current, ...range }))} onChange={mediaAccounts => setFolder(current => ({ ...current, mediaAccounts }))} /> : false ? (
