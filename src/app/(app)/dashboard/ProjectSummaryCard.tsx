...
export default function ProjectSummaryCard({ data, title, channelColors, onChannelColorChange }: ProjectSummaryCardProps) {
   ...
   */
  const statBoxClass = "flex min-h-[174px] min-w-[190px] flex-1 flex-col overflow-hidden rounded-2xl border border-border bg-secondary/20 px-3 py-3 print:min-h-0 print:min-w-0 print:rounded-xl print:px-2 print:py-2";
    ...
    <div className="rounded-2xl border border-border bg-background p-5 print:rounded-xl print:p-2.5">
      ...
      */}
      ...
      {/*
        ...
        4개/5개로 데이터마다 달라 트랙 계산이 어긋나기 쉬웠다 — 고정 5열(grid-cols-5,
        각 열 minmax(0,1fr))로 바꿔 항상 폭 계산이 같게 한다. 칸이 4개뿐이면 5번째
