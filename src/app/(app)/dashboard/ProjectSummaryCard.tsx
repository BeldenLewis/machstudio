...
export default function ProjectSummaryCard({ data, title, channelColors, onChannelColorChange }: ProjectSummaryCardProps) {
   ...
   */
  const statBoxClass = "flex min-h-[174px] min-w-[190px] flex-1 flex-col overflow-hidden rounded-2xl border border-border bg-secondary/20 px-3 py-3 print:min-h-0 print:min-w-0 print:rounded-xl print:px-1.5 print:py-1.5";
    ...
    <div className="rounded-2xl border border-border bg-background p-5 print:rounded-xl print:p-2.5">
      ...
      */}
      ...
      {/*
        ...
        겹치는 것보다 훨씬 안전하다. 첫 열(유입경로)만 1.6fr, 나머지 0.85fr.
        다섯 칸 전부 minmax(0, ...) 로 감싸는 게 핵심이다 — 그냥 "1.6fr 0.85fr ..." 만 쓰면
        내용이 많은 칸(예: 누적 사전등록자의 D-day 박스+진행바)이 자기 콘텐츠 최소 폭을
        그대로 요구해 fr 비율보다 더 가져가 버리고, print:min-w-0 가 걸린 유입경로 칸만
        그 압박을 흡수해 범례(자기도 min-w-0)가 0 폭까지 눌려 통째로 사라졌다(사용자
        스크린샷: 도넛만 남고 범례가 안 보임). minmax(0, Nfr) 는 콘텐츠 최소 폭을 0으로
        고정해 다섯 칸이 항상 비율대로만 나뉘게 한다.
      */}
      <div className="mt-4 flex min-w-0 flex-nowrap items-stretch gap-3 overflow-x-auto pb-1 print:mt-1.5 print:grid print:grid-cols-[minmax(0,1.6fr)_minmax(0,0.85fr)_minmax(0,0.85fr)_minmax(0,0.85fr)_minmax(0,0.85fr)] print:gap-1.5 print:overflow-visible print:pb-0">
