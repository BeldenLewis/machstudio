...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const from = bounds._min.startedAt;
  ...
  const rows = await prisma.$queryRawUnsafe<{ label: string; viewers: number }[]>(
  ...
  const points = rows.map((row) => ({ label: row.label, viewers: Number(row.viewers) }));
  ...
  return NextResponse.json(
    { points, peak, avg, bucketMinutes: Math.round(bucketSeconds / 60), hasData: points.length > 0 },
    { headers: { "Cache-Control": "private, max-age=60" } },
  );
}
