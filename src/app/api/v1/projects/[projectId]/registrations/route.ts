...
export async function GET(request: Request, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  try {
    ...
    if (!report) return apiJson({ error: { code: "not_found", message: "등록 데이터를 찾을 수 없습니다." } }, { status: 404 });
    return apiJson({ data: {
      generatedAt: report.generatedAt,
      project: report.project,
      range: report.range,
      performance: report.performance,
      trend: report.cumulativeTrend,
      fieldDistributions: report.fieldStats,
      duplicateSummary: report.dedup,
