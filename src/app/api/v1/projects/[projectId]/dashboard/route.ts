...
export async function GET(request: Request, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  const project = await requireApiProject(auth.principal.workspaceId, projectId);
  if (!project) return apiJson({ error: { code: "not_found", message: "프로젝트를 찾을 수 없습니다." } }, { status: 404 });
  try {
    const url = new URL(request.url);
    const range = parseApiDateRange({ from: url.searchParams.get("from"), to: url.searchParams.get("to") });
    const data = await getApiDashboard({ workspaceId: auth.principal.workspaceId, projectId, range });
    if (!data) return apiJson({ error: { code: "not_found", message: "대시보드 데이터를 찾을 수 없습니다." } }, { status: 404 });
    return apiJson({ data });
  } catch (error) {
