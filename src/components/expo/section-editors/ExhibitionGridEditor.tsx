    ...
    <div data-testid="exhibition-grid-editor" className="min-w-0 space-y-3">
      ...
      <InlineEditableTable
        ...
        renderRow={(row, index) => <div className="grid min-w-0 gap-2">
          ...
          <ExpoMediaUploadField siteId={props.siteId} kind="image" label={`${index + 1}번 하위 전시 심볼`} value={row.symbol} disabled={!props.canEdit} onChange={(next) => patchRow(index, { symbol: next?.kind === "image" ? next : undefined })} />
