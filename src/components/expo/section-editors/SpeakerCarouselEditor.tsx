    ...
    <div data-testid="speaker-carousel-editor" className="min-w-0 space-y-3">
      ...
      <InlineEditableTable
        ...
        renderRow={(row, index) => <div className="grid min-w-0 gap-3">
          ...
          <ExpoMediaUploadField siteId={props.siteId} kind="image" label={`${index + 1}번 연사 이미지`} fieldPath={`[${index}].image.url`} value={row.image} disabled={!props.canEdit} onChange={(next) => patchSpeaker(index, { image: next?.kind === "image" ? next : undefined })} />
