          ...
          <fieldset key={group.audience} disabled={!props.canEdit} className={`${R.surface} ${FINISH.s2} min-w-0 space-y-2 bg-secondary/40 p-3`}>
            ...
            <InlineEditableTable
              ...
              renderRow={(row, index) => <div className="grid min-w-0 gap-2">
                ...
                <ExpoMediaUploadField siteId={props.siteId} kind="image" label={`${index + 1}번 링크 아이콘`} value={row.icon} disabled={!props.canEdit} onChange={(next) => patchRow(index, { icon: next?.kind === "image" ? next : undefined })} />
